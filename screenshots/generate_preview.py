...
def draw_menubar(img):
    ...
    for sz in [3, 6, 9]:
        draw.arc(
            [wc - sz, 20 - sz, wc + sz, 20 + sz], start=200, end=340, fill=(200, 200, 220), width=1
        )
...
def window_frame(img, x, y, w, h, title, radius=14, accent=None):
    ...
    draw.rounded_rectangle([x, y, x + w, y + h], radius=radius, outline=(*accent, 60), width=1)
    draw.rounded_rectangle(
        [x + 1, y + 1, x + w - 1, y + h - 1], radius=radius - 1, outline=(255, 255, 255, 8), width=1
    )
...
def draw_code_editor(img, x, y, w, h):
    ...
    tabs = [
        ("intent_engine.py", True),
        ("model_router.py", False),
        ("config.toml", False),
        ("axon_core.py", False),
    ]
    tx2 = x + 10
    ...
    d.rectangle(
        [x + lnw, cy2 + 8 + 14 * line_h - 1, x + w - 10, cy2 + 8 + 15 * line_h - 1],
        fill=(25, 20, 50),
    )
    ...
    code = [
        ...
        [
            ...
            (P["text"], " axon.ai "),
            (P["ck"], "import"),
            (P["text"], " IntentParser, ModelRouter"),
        ],
        [
            (P["ck"], "from"),
            (P["text"], " axon.shell "),
...
def draw_terminal(img, x, y, w, h):
    ...
    d.text(
        (x + 14, ty_base),
        "Axon OS 0.1.0-alpha  ·  kernel 6.8.0-axon  ·  llama3.2:3b ✓",
        font=f10,
        fill=dim_col,
    )
    row[0] += 1
    d.line(
        [(x + 10, ty_base + row[0] * lh - 4), (x + w - 10, ty_base + row[0] * lh - 4)],
        fill=(20, 30, 40),
    )
    row[0] += 1
    ...
    d.rectangle(
        [sx2, ty_base + row[0] * lh + 1, sx2 + 9, ty_base + row[0] * lh + lh - 2],
        fill=(*P["cyan"], 180),
    )
...
def draw_ai_panel(img):
    ...
    def ai_bubble(lines, yy):
        ...
        rr(
            d,
            [px + 10, yy, px + pw - 10, yy + bh],
            10,
            fill=(18, 18, 34, 220),
            outline=(40, 40, 70, 180),
            w=1,
        )
    ...
    rr(d, [px + 10, iy + 8, px + pw - 48, iy + 34], 8, fill=(18, 18, 32), outline=(40, 40, 70), w=1)
...
def draw_intent_bar(img):
    ...
    gl = glow_layer(
        img.size, bx + bw // 2, by + bh // 2, 380, 160, P["violet"], steps=100, peak_alpha=45
    )
    layer.paste(gl, (0, 0), gl)
    gl2 = glow_layer(
        img.size, bx + bw // 2, by + bh // 2, 260, 100, P["cyan"], steps=80, peak_alpha=20
    )
    ...
    for i in range(24, 0, -1):
        a = int(110 * (i / 24) ** 2)
        d.rounded_rectangle(
            [bx - i, by + i, bx + bw + i, by + bh + i], radius=20, fill=(0, 0, 0, a)
        )

    d.rounded_rectangle([bx, by, bx + bw, by + bh], radius=20, fill=(*P["win_bg"], 252))
    d.rounded_rectangle([bx, by, bx + bw, by + bh], radius=20, outline=(*P["violet"], 130), width=1)
    d.rounded_rectangle(
        [bx + 1, by + 1, bx + bw - 1, by + bh - 1], radius=19, outline=(255, 255, 255, 10), width=1
    )
    d.rounded_rectangle([bx + 2, by + 1, bx + bw - 2, by + 2], radius=19, fill=(255, 255, 255, 22))
    ...
    f11b = _font(11, True)
    badges = [
        ("● axon-3b", P["cyan"], (10, 50, 70, 200), P["cyan"]),
    ...
    for lbl, col, bg, border in reversed(badges):
        ...
        bx_r -= bw2
        rr(d, [bx_r, by + 9, bx_r + bw2, by + 32], 9, fill=bg, outline=(*border, 150), w=1)
    ...
    for chip in ["Open App", "Run Command", "Search Web", "Write Code", "Summarize"]:
        ...
        rr(
            d,
            [cx6, cy6, cx6 + cw2, cy6 + 22],
            8,
            fill=(*P["chip"], 200),
            outline=(*P["chip_border"], 180),
            w=1,
        )
...
def draw_dock(img):
    ...
    for i in range(16, 0, -1):
        a = int(55 * (i / 16) ** 2)
        d.rounded_rectangle(
            [dock_x - i, dock_y + i, dock_x + dock_w + i, dock_y + dock_h + i],
            radius=36,
            fill=(0, 0, 0, a),
        )

    d.rounded_rectangle(
        [dock_x, dock_y, dock_x + dock_w, dock_y + dock_h], radius=36, fill=(*P["dock_bg"], 235)
    )
    d.rounded_rectangle(
    ...
    d.arc(
        [dock_x + 6, dock_y + 3, dock_x + dock_w - 6, dock_y + dock_h - 3],
        start=200,
        end=340,
        fill=(255, 255, 255, 22),
        width=1,
    ...
    for name, color, sym, running in ICONS:
        if name is None:
            d.line(
                [(ix + 6, dock_y + 16), (ix + 6, dock_y + dock_h - 16)],
                fill=(*P["sep"], 180),
                width=1,
            )
            ix += sep_w
        ...
        if running:
            gl = glow_layer(layer.size, icx, icy - lift, 50, 50, color, steps=40, peak_alpha=55)
            layer.paste(gl, (0, 0), gl)
            d = ImageDraw.Draw(layer)
        ...
        d.text(
            (icx - (bb[2] - bb[0]) // 2, icy - lift - (bb[3] - bb[1]) // 2 - 2),
            sym,
            font=fs,
...
def draw_watermark(img):
    ...
    tcx(
        d,
        "Axon OS 0.1.0-alpha  ·  GNOME on Ubuntu Noble  ·  Futuristic Edition",
        W // 2,
        H - 14,
        f,
        P["text_ghost"],
